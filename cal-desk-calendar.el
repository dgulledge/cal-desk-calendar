...
(defvar diary-schedule-daylight-savings-time-pattern "^[ 	]*\\(Daylight Savings Time Begins\\|Daylight Savings Time Ends\\) \\([0-9]?[0-9]\\):\\([0-9][0-9]\\)\\([ap]\\)m\\> ([A-Z0-9+-]*)"
  "*Pattern to match for daylight savings time diary entries")
	  ...
	  (t
	   (let ((start-column (1+ (length diary-schedule-time-display-format))))
		 ...
		 (insert diary-schedule-entry-separator-string
			 ...
			 text)
		 (diary-schedule-sort-entries-on-line)))
	     (while (< line end-line)
	       (forward-line 1)
	       (end-of-line)
		   ...
		   (progn
		     (insert diary-schedule-appointment-separator " "
			     diary-schedule-entry-separator-string)
		     (diary-schedule-sort-entries-on-line)))
	       (setq line (1+ line)))
...
(defun diary-schedule-sort-entries-on-line ()
  "Sort the entries on the current line by time."
  (save-excursion
    (beginning-of-line)
    (let* ((start (point))
	   (orig-start start)
      ...
      (goto-char start)
      (while (search-forward diary-schedule-entry-separator-string end t)
	(setq line
	      (cons
	       (buffer-substring-no-properties
		start
		(- (point) (length diary-schedule-entry-separator-string)))
	       line))
	(setq start (point)))
      (setq line (cons (buffer-substring-no-properties start end)
		       line))
      (setq line (sort line 'diary-schedule-compare-times))
      (delete-region orig-start end)
      (insert (car line))
      ;; (message "Inserting \"%s\"" (car line))
      (setq line (cdr line))
      (while line
	(insert diary-schedule-entry-separator-string (car line))
	;; (message "Inserting \"%s\"" (car line))
	(setq line (cdr line))))))

(defun diary-schedule-compare-times (string1 string2)
  "Compare the times on two separate entries."
  (or (< (length string1) 3)
      (if (< (length string2) 3)
	  nil
	(string-lessp (substring string1 3)
		      (substring string2 3)))))
  ...
  (if (or (not diary-entries-list)
    ...
    (save-excursion;; Prepare the fancy diary buffer.
      ...
      (while (re-search-forward
	      (concat diary-schedule-entry-separator-string "\\n?") nil t)
	(if (/= (char-before) ?\n)
	    (replace-match "\n" nil nil)))
